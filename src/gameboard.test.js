...
describe("Gameboard.placeShip", () => {
  ...
  test("throws error for positions that aren't contiguous and aligned", () => {
    ...
    ).toThrow("Ship positions are not contiguous and aligned in placeShip input");
  ...
  test("throws error for invalid positions in positions array", () => {
    ...
    ).toThrow("Invalid positions in positions array given to placeShip function");
  ...
  test("throws error for occupied positions", () => {
    ...
    ).toThrow("Occupied position in positions array given to placeShip function");
  });

  test("places a ship successfully on valid input", () => {
    board.placeShip("destroyer", [[1, 1], [1, 2]]);
    const ship = board.board[1][1].ship ? board.board[1][1].ship : board.board[1][1];
    expect(ship).toBeInstanceOf(Ship);
    // Check the same ship instance is at all positions
    expect(board.board[2][1].ship ? board.board[2][1].ship : board.board[2][1]).toBe(ship);
  });

  test("places the same Ship instance in all specified positions", () => {
    const positions = [[3, 3], [3, 4]];
    board.placeShip("destroyer", positions);
    const ship = board.board[3][3].ship ? board.board[3][3].ship : board.board[3][3];
    expect(ship).toBe(board.board[4][3].ship ? board.board[4][3].ship : board.board[4][3]);
  });

  test("places ship successfully at edge of board", () => {
    board.placeShip("destroyer", [[9, 8], [9, 9]]);
    expect(board.board[8][9].ship ? board.board[8][9].ship : board.board[8][9]).toBeInstanceOf(Ship);
    expect(board.board[9][9].ship ? board.board[9][9].ship : board.board[9][9]).toBeInstanceOf(Ship);
  });

  test("allows placing multiple different ships on non-overlapping positions", () => {
    board.placeShip("destroyer", [[0, 0], [0, 1]]);
    board.placeShip("submarine", [[2, 2], [2, 3], [2, 4]]);
    expect(board.board[0][0].ship ? board.board[0][0].ship : board.board[0][0]).toBeInstanceOf(Ship);
    expect(board.board[2][2].ship ? board.board[2][2].ship : board.board[2][2]).toBeInstanceOf(Ship);
  });
});